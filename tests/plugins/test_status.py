...
def test_status_shows_all_targets(cli_runner, local_backend):
    result = cli_runner.invoke(main, ["status"])
    assert "Target2" in result.output
    assert "Target1" in result.output


def test_status_shows_one_named_target(cli_runner, local_backend):
    result = cli_runner.invoke(main, ["status", "Target1"])
    assert "Target2" not in result.output
    assert "Target1" in result.output


def test_status_shows_two_named_targets(cli_runner, local_backend):
    result = cli_runner.invoke(main, ["status", "Target1", "Target2"])
    assert "Target2" in result.output
    assert "Target1" in result.output


def test_status_shows_only_endpoint_targets(cli_runner, local_backend):
